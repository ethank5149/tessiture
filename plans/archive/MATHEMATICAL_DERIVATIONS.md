...
### Problem Statement
For each reported inferential metric, estimate the statistic from observed voiced data, construct a two-sided percentile bootstrap confidence interval, and compute a two-sided bootstrap p-value against a preset-dependent null value.

### 8.1 Bootstrap Estimator Setup and Notation
...
- `f0_mean_hz`: `T_m(x) = mean(x)` on voiced F0 values (Hz)
- `f0_min_hz`: `T_m(x) = min(x)` on voiced F0 values (Hz)
- `f0_max_hz`: `T_m(x) = max(x)` on voiced F0 values (Hz)
- `tessitura_center_midi`: `T_m(x) = mean(x)` on voiced MIDI values
- `pitch_error_mean_cents`: `T_m(x) = mean(x)` on per-frame pitch error values (cents)
...
- `B = max(200, TESSITURE_BOOTSTRAP_SAMPLES)` (default environment value is 1000)
...
- If `n = 0`: estimate, CI bounds, and p-value are `null`
- If `n = 1`: bootstrap distribution is degenerate at `θ̂_m`, so CI low = high = estimate

### 8.2 Percentile Confidence Interval at Level `1 - α`

Let confidence level be:
```
c = 1 - α
```
For a two-sided percentile interval, use the empirical quantiles of bootstrap replicates:
```
...
### 8.3 Two-Sided Bootstrap p-Value Against a Null Value
...
This is exactly the implemented tail-doubling rule with clipping to `[0, 1]`.

### 8.4 Metric-Specific Null Hypotheses by Preset

The active preset (`casual`, `intermediate`, or `vocalist`) determines the null values `θ0,m` for all inferential metrics:
...
1. Read `metadata.inferential_preset` if provided.
2. Normalize to lowercase string.
3. If unknown, fall back to `DEFAULT_INFERENTIAL_PRESET` (environment-defined, default `casual`).
4. If fallback is also invalid, use `casual`.

Changing preset changes null-hypothesis values and therefore p-values, but does **not** change observed estimates or bootstrap CI construction from the same data.

### 8.5 Practical Interpretation Guidance

- **Estimate** is the observed sample statistic for that metric.
- **Confidence interval** summarizes sampling uncertainty under bootstrap resampling of the observed data; narrower intervals imply more stable estimates under this resampling model.
- **p-value** quantifies how extreme the preset null value is relative to the bootstrap distribution of the statistic.
- If the null value is well outside the central bootstrap mass, p-values tend to be small and often align with CIs that exclude the null.
- Interpret metrics independently (`f0_mean_hz`, `f0_min_hz`, `f0_max_hz`, `tessitura_center_midi`, `pitch_error_mean_cents`); no multiple-testing correction is currently applied in the API payload.
- For small `n_samples`, especially extrema metrics (`f0_min_hz`, `f0_max_hz`), inferential outputs can be unstable and should be treated cautiously.

### 8.6 Implementation Traceability

Backend implementation anchors:
- Null presets and defaults: `INFERENTIAL_NULL_PRESETS`, `DEFAULT_INFERENTIAL_PRESET`
- Preset resolution: `_resolve_inferential_preset(...)`
- Two-sided p-value routine: `_bootstrap_two_sided_p_value(...)`
- Per-metric inference payload builder: `_build_metric_inference(...)`
- Suite orchestration and output schema: `_build_inferential_statistics(...)`
- Response inclusion point: `results["inferential_statistics"]`

Frontend exposure anchors:
- Consumer object: `results.inferential_statistics`
- Metrics iteration: `Object.entries(inferentialStatistics.metrics)`
- UI table columns: metric, estimate, CI, p-value, and `n_samples`
- Display location: inferential section in `AnalysisResults`

**Citation:** Efron, B. & Tibshirani, R.J. (1993). *An Introduction to the Bootstrap*
...
These derivations enable independent verification of the system's mathematical correctness and provide a foundation for future enhancements, including inferential interpretation of per-metric bootstrap outputs.
