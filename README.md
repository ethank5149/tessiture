...
### Release version metadata flow (canonical source)

- Canonical release version source: `/.release-version` at repo root.
- The file is generated by `deploy/unraid/scripts/build.sh` whenever a semantic release version is resolved.
- `--version-bump none` behavior in `build.sh`:
  - if the selected image tag is semantic (`x.y.z` or `vx.y.z`), that version is written to `/.release-version`.
  - if the image tag is non-semantic, no invalid value is written; the script logs this and clears stale `/.release-version` when present.
- Frontend release metadata is injected at build time via `VITE_APP_VERSION` (Docker build arg/env path and Makefile `build-frontend` path); frontend UI displays it only when semantic.
- `deploy/unraid/scripts/one-shot.sh` reads `/.release-version` after build and syncs deploy metadata in env as `TESSITURE_RELEASE_VERSION`.
- `TESSITURE_IMAGE` remains the deployment image source-of-truth for compose deploys.
